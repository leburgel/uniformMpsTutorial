...
function [lam, Fr] = rightFixedPointMPO(O, Ar, tol)
    ...
    % construct handle for the action of the relevant operator
    transferRightHandleMPO = @(v) reshape(ncon({reshape(v, [D d D]), Ar, conj(Ar), O}, {[1, 3, 5], [-1, 2, 1], [-3, 4, 5], [-2, 2, 3, 4]}), [], 1);
    [Fr, lam] = eigs(transferRightHandleMPO, D^2*d, 1, 'largestabs', 'Tolerance', tol); % right eigenvector
    Fr = reshape(Fr, [D d D]);
end
...
function [AcTilde, CTilde] = calcNewCenterMPO(O, Ac, C, Fl, Fr, lam, tol)
    ...
    handleAc = @(X) reshape(O_Ac(reshape(X, [D d D]), O, Fl, Fr, lam), [], 1);
    handleC = @(X) reshape(O_C(reshape(X, [D D]), Fl, Fr), [], 1);
    % solve eigenvalue problem using 'largest magnitude' option
    [AcTilde, ~] = eigs(handleAc, D^2*d, 1, 'largestabs', 'Tolerance', tol, 'StartVector', reshape(Ac, [], 1)); % variable tolerance
    AcTilde = reshape(AcTilde, [D d D]);
    [CTilde, ~] = eigs(handleC, D^2, 1, 'largestabs', 'Tolerance', tol, 'StartVector', reshape(C, [], 1)); % variable tolerance
...
function [lam, Al, Ac, Ar, C, Fl, Fr] = vumpsMPO(O, D, A0, tol)
    ...
    while flag
        ...
        [~ , Fr] = rightFixedPointMPO(O, Ar, delta/10);
        ...
        delta = ArrayNorm(O_Ac(Ac, O, Fl, Fr, lam) - ncon({Al, O_C(C, Fl, Fr)}, {[-1 -2 1], [1 -3]}));
        if delta < tol
            flag = false;
        end
        % update tensors
